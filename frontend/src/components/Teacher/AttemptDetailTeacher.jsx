...
const AttemptDetailTeacher = () => {
  ...
  const loadAttemptDetail = async () => {
    try {
      const data = await attemptService.getAttemptDetailForTeacher(attemptId);
      setAttempt(data);
    } catch (error) {
  ...
  return (
    <div className="container">
      <div className="card">
        <h2>Детали попытки студента</h2>
        <div style={{ marginBottom: '20px' }}>
          <p><strong>Студент:</strong> {attempt.student_name}</p>
...
export default AttemptDetailTeacher;